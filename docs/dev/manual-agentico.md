...
| **El log de uso** | `npm run revisar:uso` (`-- --ver <id>`, `-- --id <id> --veredicto …`) · `lib/uso.ts` · `lib/mcp/registro-de-uso.ts` | `agent_log` | Qué se le pidió a la app y qué contestó, **en las dos superficies**. La columna que más importa es `decisiones`: sin ella el log dice qué contestó pero no **por qué**. ⚠ **`registrarUso()` nunca lanza** (regla 7: si el log falla, la respuesta sale igual) **pero sí se espera** — sin `await`, en Vercel la fila se pierde a veces sí y a veces no, y un log con agujeros irregulares es peor que ninguno. ⚠ El conector se instrumenta envolviendo el `server` **una sola vez**, así una tool nueva queda registrada por existir. ⚠ **Es la única tabla que NO va al respaldo**: diagnóstico, no dominio. |
